...
namespace Final_Progect
{
    public partial class gameEasy : Form
    {
        ...
        private void timer2_Tick(object sender, EventArgs e)
        {
            ...
            baseTop = picBase.Top;
            baseLeft = picBase.Left;
            baseWidth = picBase.Width;
            baseHeight = picBase.Height;
            ...
            if (((shutleLeft >= baseLeft && shutleLeft <= baseRight) ||
                 (shutleRight >= baseLeft && shutleRight <= baseRight)) &&
                ((shutleTop >= baseTop && shutleTop <= baseBottom) ||
                 (shutleBottom >= baseTop && shutleBottom <= baseBottom)))
            {
                lblLife.Text = "lives: ";
            ...
            else if (((baseLeft >= shutleLeft && baseLeft <= shutleRight) ||
                      (baseRight >= shutleLeft && baseRight <= shutleRight)) &&
                     ((baseTop >= shutleTop && baseTop <= shutleBottom) ||
                      (baseBottom >= shutleTop && baseBottom <= shutleBottom)))
            {
                lblLife.Text = "lives: ";
        ...
        private void tmShoot_Tick(object sender, EventArgs e)
        {
            ...
            if (((buletLeft >= rockLeft && buletLeft <= rockRight) ||
                ...
                ((buletTop >= rockTop && buletTop <= rockBottom) ||                 //get the bulet to fallow the shutle
